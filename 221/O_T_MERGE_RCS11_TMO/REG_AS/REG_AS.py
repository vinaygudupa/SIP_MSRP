...
DEFAULT_FEATURES = file_lines[4].split('->')[1].strip()
HTTP_FT_FEATURES = file_lines[5].split('->')[1].strip()
...
for i in file_lines:
    if i.startswith("range_"):
        ...
        tag = data[4].strip()
        dict_range_data[name] = [start_num,(start_num+range_num-1),scscf,tag]
    else:
        continue
                ...
                if i == 'REG':
                    ...
                    for j in range(len(dict_range_data[i][2])):
                        ...
                        if j == 0:
                            ...
                            sip_instance = "urn:uuid:11111111-1111-1111-1111-aa" + destNum
                        elif j == 2:
                            sip_instance = "urn:uuid:22222222-2222-2222-2222-aa" + destNum

                        if tag == 'HTTP_FT_FEATURE_TAG':
                            FEATURES = HTTP_FT_FEATURES
                        else:
                        ...
                        contact = contact + '{\r\n"id":"' + str(j) + '",\r\n"RecRegTime":"' + str(int(time.time())-100+j) \
                                  + '",\r\n"sip_uri":"sip:' + destNum + "@" + ip_port[1] + ":" + ip_port[2] + '",\r\n' \
                                  + '"sip_instance":"' + sip_instance + '",\r\n' \
                                  + '"SuppFork":"1",\r\n' \
                                  + '"feature_tags":"' + feature_tags + '"\r\n},'
                    contact = contact[:-1]
                    tp_data_base64 = convert_to_base64(tp_data + contact + '\r\n]\r\n}\r\n}}')
                    response_final = '{"rsp": {"regData": [{"versionNum": "0", "destUserId": "1' + destNum \
                                    + '", "tpDataType": "base64", "tpData": "' + tp_data_base64 \
                                    + '", "regInfoTTL": "3600", "nodeType": "RCS"}]}}'
                    GET_200 += 1
