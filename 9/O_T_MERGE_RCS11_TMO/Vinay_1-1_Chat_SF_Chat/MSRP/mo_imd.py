...
UDP_PORT = int(file_lines[3].split('=')[1].strip())
RMS_HOST = file_lines[4].split('=')[1].strip()
msrp_message_to_be_sent = int(file_lines[5].split('=')[1].strip())
task_timeout = int(file_lines[6].split('=')[1].strip())
delay_between_successive_messages = float(file_lines[7].split('=')[1].strip())
fqdn = file_lines[8].split('=')[1].strip()
app_serv_port_start_from_file = file_lines[9].split('=')[1].strip().rstrip(']').lstrip('[').split(',')
for p in range(len(app_serv_port_start_from_file)):
    app_serv_port_start.append(int(app_serv_port_start_from_file[p]))
app_serv_port_range = int(file_lines[10].split('=')[1].strip())
msrp_data_length = int(file_lines[11].split('=')[1].strip())
