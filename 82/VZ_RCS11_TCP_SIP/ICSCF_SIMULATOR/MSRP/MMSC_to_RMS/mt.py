...
RMS_HOST = file_lines[2].split('=')[1].strip()
task_timeout = int(file_lines[3].split('=')[1].strip())
fqdn = file_lines[4].split('=')[1].strip()
app_serv_port_start_from_file = file_lines[5].split('=')[1].strip().rstrip(']').lstrip('[').split(',')
for p in range(len(app_serv_port_start_from_file)):
    app_serv_port_start.append(int(app_serv_port_start_from_file[p]))
app_serv_port_range = int(file_lines[6].split('=')[1].strip())
image_file = file_lines[7].split('=')[1].strip()
CHUNK_SIZE = int(file_lines[8].split('=')[1].strip())
...
async def recv_msrp_data(reader, writer, fromTag, toTag, from_num, to_num):
    rcvd_packet_count = 0
...
async def msrp_handler(fromTag, toTag, toPort, from_num, to_num, callNum):
    ...
    try:
        ...
        if reader and writer:
            dict_call_num[call_num][2] = writer
            result = await recv_msrp_data(reader, writer, fromTag, toTag, from_num, to_num)
            if result :
