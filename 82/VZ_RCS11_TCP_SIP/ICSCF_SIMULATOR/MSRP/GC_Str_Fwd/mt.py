...
orig_num_starts_with = file_lines[10].split('=')[1].strip()
...
async def connect(remote_port, fromTag, toTag):
    ...
    try:
        ...
        print("Exception in connect : " + str(e) + "    port: " + str(remote_port))
        return ['', '']
...
async def msrp_data_recv(reader, writer, fromTag, toTag, fromNum, toNum):
    if not str(fromNum)[1:].startswith(orig_num_starts_with):
        return True 
    imdn_count = 2
    imdn_recvd = 0
    for i in range(imdn_count):
        ...
        try:
            result = await read_search(reader, MSRP_end_char, search_str, True, False)
        except asyncio.CancelledError as e:
            return False
