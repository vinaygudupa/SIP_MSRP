...
task_timeout = int(file_lines[4].split('=')[1].strip())
fqdn = file_lines[5].split('=')[1].strip()
app_serv_port_start_from_file = file_lines[6].split('=')[1].strip().rstrip(']').lstrip('[').split(',')
for p in range(len(app_serv_port_start_from_file)):
    app_serv_port_start.append(int(app_serv_port_start_from_file[p]))
app_serv_port_range = int(file_lines[7].split('=')[1].strip())
Large_Data = "a" * int(file_lines[8].split('=')[1].strip())
CHUNK_SIZE = int(file_lines[9].split('=')[1].strip())
...
async def recv_msrp_data(reader, writer, fromTag, toTag, from_num, to_num):
    ...
    while(rcvd_packet_count < num_of_chunks_to_recv):
        ...
        while True:
            try:
                temp = await reader.read(1600000)
                data_read += temp
